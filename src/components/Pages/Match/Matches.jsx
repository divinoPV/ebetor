  ...
  useEffect(() => (async () => {
    ...
    axios.get(`${slugGame}/matches?sort=begin_at&page=${~~(itemOffset / itemsPerPage) + 1}&per_page=${itemsPerPage}${statusSearch}`).then(response => {
      ...
      if (response.data.length === 0) {
        ...
        axios.get(`matches?sort=name&page=1&per_page=50${statusSearch}`).then(r => {
        ...
        <div className={`${styles['Matches__leading']}`}>
          {!authentication.logged &&
            <strong className={`${styles['Matches__leading__notLogged']}`}>
              ...
              Vous devez-etre connecté pour pouvoir parier.
            </strong>
          }
          {message &&
            <strong className={`${styles['Matches__leading__message']}`}>
              ...
              {message} Les matches de tous les jeux sont affichés en fonction du statut.
            </strong>}
        </div>
      }
      <Formik
